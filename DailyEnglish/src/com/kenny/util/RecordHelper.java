...
public class RecordHelper implements OnCompletionListener, OnErrorListener {
	static final String SAMPLE_PREFIX = "zwz测试录音";
	...
	public int getMaxAmplitude() {
		if (mState != RECORDING_STATE)
			return 0;
		return extAudioRecorder2.getMaxAmplitude();
	}
	...
	public File recordChat(ExtAudioRecorder extAudioRecorder, String savePath,
		...
		extAudioRecorder2 = extAudioRecorder;
	...
	public void startRecording(int outputfileformat, String extension,
		...
		if (mSampleFile == null) // 如果没有这个文件夹
		{
			File sampleDir = Environment.getExternalStorageDirectory();

			if (!sampleDir.canWrite()) // Workaround for broken sdcard support
										// on the device.
				sampleDir = new File("/sdcard/TestCeshi" + ".mp3");

			try {
				/**
				 * 1.SAMPLE_PREFIX 保存名 zwz测试录音 2.extension 后缀名 .amr 3.sampleDir
				 * 路径名 /nmt/scard
				 */
