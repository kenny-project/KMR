...
public class PopLocalMenu extends PopMenu
{
    ...
    * nFlag: 0:local,1:favor 2:kuaipan
    * */
   ...
   protected void OnItemClickListener(int pos, KString key)
   {
      ...
      case copy:
         if (file.getFile().canRead())
         {
	  SysEng.getInstance().addHandlerEvent(new copyFileEvent(context,file));
         }
         else
         {
	  Toast.makeText(context,
      ...
      case cut:
         if (file.getFile().canRead())
         {
	  SysEng.getInstance().addHandlerEvent(new cutFileEvent(context,file));
...
//         }
         break;
      case favor:
         SysEng.getInstance().addEvent(new FavoriteFileEvent(context, file, 0));
         break;
      case zip:
         if (file.getFile().canRead())
         {
