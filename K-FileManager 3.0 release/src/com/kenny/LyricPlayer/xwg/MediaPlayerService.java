      ...
      MediaPlayer.OnCompletionListener, MusicFocusable
{
   ...
   public class LoadData extends AbsEvent
   {
	  ...
	  count = 1;
         ...
         mProgressDialog = ProgressDialog.show(MediaPlayerService.this, "",
	     "正在加载数据...", true, true);
      }
	        ...
	        )
	  {
         ...
         Collections.sort(mMusicList, new LyricFileNameSort());
         for (int i = 0; i < mMusicList.size(); i++)
         {
	  String fileName = mMusicList.get(i).getMusic_title();
	  if (mFileName.equals(fileName))
	  {
	     P.v("mFileName=" + mFileName + ",fileName=" + fileName + ",i="
		 + i);
	  ...
	  public void ok()
	  {
	     try
	     {
	        ...
	        // if (mDataSource != mMediaInfoProvider.getUrl()
	        // && (isStop() == false))
	        // {
	        // stop();
	        // }
	     ...
	     {
	        e.printStackTrace();
	        Toast.makeText(MediaPlayerService.this, "内存不足加载失败",
   ...
   public interface LyricPlayerListener
   {
      ...
      public void onStateChanged(int nState);// 播放器状态 0:init 1:play 2:pause 3:stop
   }
...
//      }
   ...
   public void onGainedAudioFocus()
   {
      ...
      if (isPausing()) configAndStartMediaPlayer();
   }
   
   public void onLostAudioFocus()
   {
      // start/restart/pause media player with new focus settings
      if (mMediaPlayer != null && mMediaPlayer.isPlaying()) configAndStartMediaPlayer();
   }
   ...
   void configAndStartMediaPlayer()
   {
      ...
      else if (mAudioFocusHelper.getAudioFocus() == AudioFocusHelper.NoFocusCanDuck) mMediaPlayer
	  .setVolume(DUCK_VOLUME, DUCK_VOLUME); // we'll
	                                        // be
	                                        // relatively
	                                        // quiet
      else mMediaPlayer.setVolume(1.0f, 1.0f); // we can be loud
