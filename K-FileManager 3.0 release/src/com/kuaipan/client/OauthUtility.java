...
public class OauthUtility
{
   ...
   public static KuaipanURL buildPostURL(String host, String location,
         Map<String, String> params, Consumer consumer, TokenPair token,
         boolean isSecure)
   {
   ...
   private static KuaipanURL buildURL(String method, String host,
         String location, Map<String, String> params, Consumer consumer,
         TokenPair token, boolean isSecure)
   {
      
      TreeMap<String, String> signed_params;
      location = urlEncode(location);
      if (params != null) signed_params = new TreeMap<String, String>(params);
      else signed_params = new TreeMap<String, String>();
      ...
      try
      {
         signature = generateSignature(method, host, location, signed_params,
      ...
      if (token != null) signed_params.put("oauth_token", token.key);
   ...
   private static String encodeParameters(Map<String, String> signed_params)
   {
      ...
      for (Iterator<String> iter = signed_params.keySet().iterator(); iter
	  .hasNext();)
      {
	  ...
	  // never come here
         ...
         if (iter.hasNext()) buf.append(JOIN_AND);
      }
      return buf.toString();
   ...
   private static String generateSignature(String method, String host,
         String location, TreeMap<String, String> params, Consumer consumer,
         TokenPair token, boolean isSecure) throws NoSuchAlgorithmException,
         InvalidKeyException
   {
      
      String msg = generateBaseString(method, host, location, params, consumer,
	  token, isSecure);
   ...
   private static String generateSecret(Consumer consumer, TokenPair token)
   {
      ...
      buf.append(JOIN_AND);
      if (token != null) buf.append(token.secret);
         ...
         TokenPair token, boolean isSecure)
   {
      ...
      buf.append(method);
      buf.append(JOIN_AND);
      if (isSecure) buf.append(oauthEncode("https://" + host + location));
      else buf.append(oauthEncode("http://" + host + location));
         ...
         Consumer consumer, TokenPair token)
   {
      ...
      if (token != null) tm.put("oauth_token", token.key);
   ...
   private static String oauthEncode(String str)
   {
      try
      {
         return URLEncoder.encode(str, ENCODE).replace("*", "%2A")
	     .replace("%7E", "~").replace("+", "%20");
      }
      catch (UnsupportedEncodingException e)
      {
         return null;
