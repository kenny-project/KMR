...
public class Dao extends DBHelper
{
	...
	public void InsertFavorites(List<FavorBean> favorItems)
	{
		synchronized (_readLock)
		{
			...
			String sql = "";
			sql = "insert into " + DBHelper.favTable + "("
					+ DBHelper.favFilds[1] + "," + DBHelper.favFilds[2] + ","
					+ DBHelper.favFilds[3] + "," + DBHelper.favFilds[4]
					+ ") values (?,?,?,?)";
	...
	public void saveInfos(List<AppBean> infos)
	{
		synchronized (_readLock)
		{
			...
			{
				Object[] bindArgs =
				{ info.getAppName(), info.getPackageName(),
						info.getVersionName(), info.getVersionCode(),
						info.getFilePath(), info.getFlags(),
						info.getCodeSize(), info.getDataSize(),
						info.getCacheSize() };
				database.execSQL(sql, bindArgs);
			}
		}
...
//	}
	 ...
	 */
	public List<FileBean> getFavoritesInfos(FGroupInfo groupInfo)
	{
		synchronized (_readLock)
		{
			List<FileBean> list = new ArrayList<FileBean>();
		...
		synchronized (_readLock)
		{
			...
			} else
			{
				sql = "select " + DBHelper.favFilds[0] + ", "
						+ DBHelper.favFilds[1] + ", " + DBHelper.favFilds[2]
						+ " from " + DBHelper.favTable + " where "
						+ DBHelper.favFilds[1] + "=? and "
						+ DBHelper.favFilds[4] + " LIKE ? ORDER BY "
						+ DBHelper.favFilds[4] + " COLLATE LOCALIZED";
				search = search + "%";
				cursor = database.rawQuery(sql, new String[]
				{ flags, search });
			...
			while (cursor.moveToNext())
			{
				...
				if (mCurrentFile.isDirectory())
				{
					...
					if (temp != null)
					{
						bean.setItemFileCount(temp.length);
					}
				}
				bean.setLength(mCurrentFile.length());
	...
	public AppBean getAppInfo(String packageName)
	{
		synchronized (_readLock)
		{
			...
			String sql = "select " + fields
					+ " from App_info where packageName=?";
			Cursor cursor = database.rawQuery(sql, new String[]
			{ packageName });
			if (cursor.moveToNext())
			{
				info = new AppBean(context, cursor.getInt(0),
	...
	public List<AppBean> getAppInfos(List<String> packageName)
	{
		synchronized (_readLock)
		{
			...
			for (int i = 0; i < packageName.size(); i++)
			{
				...
				if (cursor.moveToNext())
				{
					AppBean info = new AppBean(context, cursor.getInt(0),
							cursor.getString(1), cursor.getString(2),
							cursor.getString(3), cursor.getInt(4),
							cursor.getString(5), cursor.getInt(6),
	...
	public List<AppBean> getAppInfos(String flags)
	{
		synchronized (_readLock)
		{
			...
			fields += DBHelper.appInfoFilds[DBHelper.appInfoFilds.length - 1];
			String sql = "select " + fields + " from App_info where flags=? order by "+DBHelper.appInfoFilds[1];
			Cursor cursor = database.rawQuery(sql, new String[]
			{ flags });
			while (cursor.moveToNext())
			{
	...
	public int delete(String packageName)
	{
		...
		{
			SQLiteDatabase database = this.getWritableDatabase();
			int result = database.delete("App_info", "packageName=?",
					new String[]
					{ packageName });
			database.close();
			return result;
	...
	public int deleteFavorites(int id)
	{
		...
		{
			SQLiteDatabase database = this.getWritableDatabase();
			int result = database.delete(DBHelper.favTable,
					DBHelper.favFilds[0] + "==?", new String[]
					{ String.valueOf(id) });
			database.close();
			return result;
		}
	 ...
	 */
	/**
	 * 查看数据库中是否有数据 nFlage:0:收藏夹 1:历史 return :true:有数据 False:没数据
	 */
	public boolean isHasHistory(int flags, String path)
	{
		...
		{
			SQLiteDatabase database = this.getReadableDatabase();
			String sql = "select count(*)  from " + DBHelper.hisTable
					+ " where flags=? and " + DBHelper.favFilds[2] + "=?";
			Cursor cursor = database.rawQuery(sql, new String[]
	...
	/**
	 * 删除指定ID的历史及收藏记录 nFlage:0:收藏 1:历史
	 */
	public int deleteHistory(int flags, String path)
	{
		...
		{
			SQLiteDatabase database = this.getWritableDatabase();
			int result = database.delete(DBHelper.hisTable, "flags=? and "
					+ DBHelper.favFilds[2] + "=?", new String[]
			{ String.valueOf(flags), path });
			database.close();
			return result;
		}
	...
	/**
	 * 添加历史记录 nFlage:0:收藏 1:历史 path:路径 size:文件大小
	 */
	public void InsertHistory(int flags, String path, long size)
	{
		synchronized (_readLock)
		{
			...
			{ String.valueOf(flags), path });
			String sql = "";
			sql = "insert into " + DBHelper.hisTable + "("
					+ DBHelper.favFilds[1] + "," + DBHelper.favFilds[2] + ","
					+ DBHelper.favFilds[3] + ") values (?,?,?)";
			Object[] bindArgs =
			{ flags, path, size };
	...
	public int deleteHistory(int id)
	{
		...
		{
			SQLiteDatabase database = this.getWritableDatabase();
			int result = database.delete(DBHelper.hisTable,
					DBHelper.favFilds[0] + "=?", new String[]
					{ String.valueOf(id) });
			database.close();
			return result;
	...
	/**
	 * 下载完成后删除数据库中的数据 nFlage:0:收藏 1:历史
	 */
