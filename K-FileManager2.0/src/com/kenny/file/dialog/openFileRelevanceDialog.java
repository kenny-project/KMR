 ...
 * 
 */
public class openFileRelevanceDialog implements OnItemClickListener,
		OnClickListener {
...
private INotifyDataSetChanged notify;
	public openFileRelevanceDialog() {
		manager = new SimpleFileManager();
	}

	public void ShowDialog(Context context, FileTypeBean bean, String path,INotifyDataSetChanged notif) {
		mContext = context;
		this.bean = bean;
		this.notify=notif;
		...
		mFolderAdapter = new FileAdapter(mContext, 1, manager.getFileList(),
				null);
		mCurrentPath.setText(manager.getCurrentPath() + bean.getTitle());
	...
	public void onItemClick(AdapterView<?> arg0, View arg1, int position,
		...
		if (temp != null) {
			...
			if (mFile.isDirectory()) {
				...
				String[] ends = "jpg|gif|png|bmp|jpeg".split("\\|");
				for (String end : ends) {
					if (end.length() > 1) {
						if (temp.getFileEnds().equals(end)) {
							bean.setPath(temp.getFilePath());
							 if(notify!=null)
							 {
							 notify.NotifyDataSetChanged(Const.cmd_OpenFile_Dialog_Result,null);
							 }
							if (alertDialog != null) {
								alertDialog.dismiss();
								alertDialog = null;
