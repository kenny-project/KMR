...
public class SoftlistView extends ObjectView implements RefreshListener,INotifyDataSetChanged
{
	...
	private boolean bRefreshLoading=false;//下拉刷新
	...
	public SoftlistView(Context context, AttributeSet attrs)
	{
		...
		mProgressDialog.setMessage(context.getText(R.string.pd_loading));
	...
	private class NetTask1 extends AsyncTask<Object, Integer, String>
	{
		...
		{
			Log.v("wmh", "doInBackground：start");
			String url = m_url + "page=" + m_list_page + "&per_page="
					+ m_everytime_num;
			String json = HttpUtil.doGet(url);
			Log.v("wmh", "doInBackground：EndsoftlistViewurl" + url);
			return json;
		}
			...
			else
			{
				try
				{
					...
					for (int i = 0; i < jsonObj1.length(); i++)
					{
						...
						tempAppListBean.setTitle(tempJson.getString("title"));// 标题
						tempAppListBean.setPn(tempJson.getString("pn"));// package码
						tempAppListBean.setLogourl(tempJson.getString("logo"));// 图片地址
						tempAppListBean.setSize(Common.getLength(tempJson
								.getString("size")));// 文件大小(字节)score
						tempAppListBean.setScore(tempJson.getString("score"));// 文件大小(字节)
						tempAppListBean.setAppurl(tempJson.getString("apkurl"));// 下载地址
						tempAppListBean.setDowntiems(tempJson.getString("dc"));// 下载次数
						tempAppListBean
