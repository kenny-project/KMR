...
//		            Defaults.getSettingsMode());
	      ...
	      String portNumber = settings.getString(PORTNUM,
		            Defaults.getPortNumber());
	      String chroot = settings.getString(CHROOTDIR,
		            Defaults.chrootDir);
	      boolean acceptNet = settings.getBoolean(ACCEPT_NET,
		            Defaults.acceptNet);
	      boolean acceptWifi = settings.getBoolean(ACCEPT_WIFI,
		            Defaults.acceptWifi);
	      boolean stayAwake = settings.getBoolean(STAY_AWAKE,
		            Defaults.stayAwake);
	      
	      // The String named chroot holds the default chroot directory.
	      // If the
	      ...
	      portNumberBox.setText(portNumber);
        ...
        public void onClick(View v)
        {
	      ...
	      validateBlock:
	      {
		    ...
		    // At least one of the wifi/net listeners must be
		    // enabled,
	      ...
	      if (errString != null)
	      {
		    ...
		    // This whole mess just adds a do-nothing "OK" button to
		    // the dialog
...
//		            Defaults.getSettingsName(),
//		            Defaults.getSettingsMode());
	      SharedPreferences settings = PreferenceManager.getDefaultSharedPreferences(this);
	      SharedPreferences.Editor editor = settings.edit();
	      
	      editor.putString(USERNAME, username);// admin
	      editor.putString(PASSWORD, password);// 111111
	      editor.putString(PORTNUM,Integer.toString(portNum)); // 8530
	      editor.putString(CHROOTDIR, chrootDir);// /
	      editor.putBoolean(ACCEPT_WIFI, acceptWifi);// true
	      editor.putBoolean(ACCEPT_NET, acceptNet); // false
	      editor.putBoolean(STAY_AWAKE, stayAwake); // false
	      editor.commit();
