...
public class SpecifyLocalFilePage extends ContentFragment implements
		...
		OnClickListener menuListener = new OnClickListener() {
			...
			public void onClick(View v) {
				...
				case R.id.btMenuListSort:
					new ViewSortDialog().ShowDialog(m_act,
							SpecifyLocalFilePage.this);
					break;
	...
	private void InstallApp() {
		...
		if (mSelFiles.size() > 0) {
			SysEng.getInstance().addEvent(
					new InstallEvent(m_act, mSelFiles,
							SpecifyLocalFilePage.this));
			return;
		}
	...
	public void onClick(View v) {
		...
		case R.id.btCut:
//			SysEng.getInstance().addHandlerEvent(
//					new cutFileEvent(m_act,localManage.getCurrentPath(),localManage.getFileList(),
//							null));
			SysEng.getInstance().addHandlerEvent(
	...
	private void SwitchStyle(int nStyle) 
	{
		...
		if (nStyle == 1) 
		{
			fileAdapter = new FileAdapter(m_act, 2, mLocalFileList, this);
			m_localGrid.setAdapter(fileAdapter);
			m_locallist.setVisibility(View.GONE);
			m_localGrid.setVisibility(View.VISIBLE);
		} else {
			fileAdapter = new FileAdapter(m_act, 1, mLocalFileList, this);
