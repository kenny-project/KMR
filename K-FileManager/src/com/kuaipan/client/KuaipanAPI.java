...
public class KuaipanAPI
{
			...
			String access_code)
	{
		try
		{
			...
			String host = KuaipanHTTPUtility.API_HOST;
			KuaipanURL url = OauthUtility.buildGetURL(host,
					location.toString(), params, session.consumer,
					session.getToken(), false);

			KuaipanHTTPResponse resp = KuaipanHTTPUtility.requestByGET(url);
			KuaipanPublicLink link = new KuaipanPublicLink(
	...
	public static KuaipanHTTPResponse documentView(ConvType type,
			ConvView view, String path, OutputStream os, ProgressListener lr)
			throws KuaipanAuthExpiredException, KuaipanServerException,
			KuaipanIOException
	{
		TreeMap<String, String> params = new TreeMap<String, String>();
	...
	public static KuaipanHTTPResponse downloadFile(String path,
			...
			KuaipanServerException
	{
		...
		if (resp.code == KuaipanHTTPResponse.KUAIPAN_AUTHORIZATION_ERROR)
		{
			if (result != null)
			{
				...
				if (msg != null
						&& msg.equals(KuaipanHTTPResponse.MSG_AUTHORIZATION_EXPIRED))
