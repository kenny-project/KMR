...
@TargetApi(9) public class KuaipanHTTPUtility
{
   ...
   public final static String AUTH_URL = "https://www.kuaipan.cn/api.php?ac=open&debug=1&op=authorise&oauth_token=";
         ...
         throws KuaipanIOException
   {
	  ...
	  if (lr != null)
	  {
	     ...
	     if ((current_time - last_triggered_time) > lr
		 .getUpdateInterval())
	     {
   ...
   public static class UploadHostFactory
   {
	  ...
	  if (resp.content == null) throw new KuaipanIOException(
	        resp.toString());
	  
	  Map<String, Object> mapresult = JSONUtility.parse(resp.content);
	  if (mapresult == null) throw new KuaipanIOException(resp.toString());
	  
	  String url = (String) mapresult.get("url");
	  if (url == null) throw new KuaipanIOException(resp.toString());
	  
	  if (url.startsWith("http://")) url = url
	        .replaceFirst("http://", "");
	  else if (url.startsWith("https://")) url = url.replaceFirst(
	        "https://", "");
