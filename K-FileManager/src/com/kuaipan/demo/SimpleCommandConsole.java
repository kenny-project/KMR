...
public class SimpleCommandConsole
{
   ...
   {
      if (!path.endsWith("/")) { return path = path + "/"; }
      return path;
   }
   ...
   public boolean execute(String cmd, String value)
   {
      try
      {
         if (cmd.equals("ls")) do_ls();
         else if (cmd.equals("cd")) do_cd(value);
         else if (cmd.equals("mkdir")) do_mkdir(value);
         else if (cmd.equals("rm")) do_rm(value);
         else if (cmd.equals("cat")) do_cat(value);
         else if (cmd.equals("exit")) return false;
      }
      catch (Exception e)
      {
         e.printStackTrace();
   ...
   public String joinPath(String dir)
   {
      ...
      else if (dir.equals(".."))
      {
         ...
         if (path_slices.length > 1) return stringJoin(path_slices, '/', 1,
	     path_slices.length - 1);
         return "/";
      }
      else
      {
   ...
   private void openBrowser(Activity context, String strUrl)
   {
	  ...
	  new NextPageEvent(context, new KuaiPanLoginPage(context, strUrl),
	        1, null));
