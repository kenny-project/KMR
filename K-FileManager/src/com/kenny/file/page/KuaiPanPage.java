...
public class KuaiPanPage extends AbsPage implements OnItemClickListener,
      ...
      OnItemLongClickListener
{
   ...
   private LinearLayout lyUpLoadTools, lyBTools;
   private List<FileBean> mFilelist;
   private String oauth_token = "";
   private String oauth_secret = "";
   
   public KuaiPanPage(Activity context)
   {
      super(context);
   ...
   public void onCreate()
   {
      ...
      lyBTools = (LinearLayout) findViewById(R.id.lyBTools);
      lyUpLoadTools = (LinearLayout) findViewById(R.id.lyUpLoadTools);
...
//	     m_act.getString(R.string.KuaiPan_use_Content));
         ...
         oauth_secret = intent.getStringExtra(KPConstants.KP_BROADCAST_SECRET);
         // 执行相应的操作
   ...
   private void getShares(final String fileName)
   {
      SysEng.getInstance().addEvent(new AbsEvent()
      {
         ...
         public void ok()
         {
	  KuaipanPublicLink mKuaiPanUser = KuaipanAPI
	        .shares(fileName, "", "");
   ...
   private void getAccountInfo()
   {
	  ...
	  if (mKuaiPanUser != null)
	  {
	     String AccountInfo = T.FileSizeToString(mKuaiPanUser.quota_used)
		 + "/" + T.FileSizeToString(mKuaiPanUser.quota_total);
	     NotifyDataSetChanged(Const.cmd_KuaiPan_AccountInfo, AccountInfo);
	  }
         }
      });
   }
   ...
   private void Refresh()
   {
      ...
      else
      {
	     ...
	     new LoadKuaiPanEvent(m_act, cli.getPath(), cli, true, this));
      }
   }
   ...
   {
      if (oauth_token.length() == 0 || oauth_secret.length() == 0) { return false; }
   ...
   private void DownLoads()
   {
	  ...
	  try
	  {
	     ...
	     Toast.makeText(m_act, "已添加到下载对队列!", Toast.LENGTH_SHORT).show();
	  }
	  catch (Exception e)
	  {
	     e.printStackTrace();
   ...
   private void deletefiles()
   {
	  ...
	  new AlertDialog.Builder(m_act)
		    ...
		    new DialogInterface.OnClickListener()
		    {
			   ...
			   for (int i = 0; i < mDelFiles.size(); i++)
			   {
			      FileBean tmpInfo = mDelFiles.get(i);
