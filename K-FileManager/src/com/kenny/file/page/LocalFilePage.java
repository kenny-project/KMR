      ...
      OnClickListener, OnItemClickListener
{
   ...
   private int nStyle = 0, nSortMode = 0; // false:listView
	                                // true:gridView
   private Button btSelectVisible, btLocalSelectAll,btToolsMemu;
   ...
   public void onCreate()
   {
	  ...
	  new LocalAddressDialog().ShowDialog(m_act);
         }
      });
      ...
      btLocalSelectAll = (Button) findViewById(R.id.btSelectAll);
      btLocalSelectAll.setOnClickListener(this);
   ...
   private void Back()
   {
	if (findViewById(R.id.ip_menu).getVisibility() == View.VISIBLE){
		new MGoneAinm(main, findViewById(R.id.ip_menu), findViewById(R.id.main_menu)).ShowAnim();
		return false;
	}
         ...
         long id)
   {
	  ...
	  if (mFile.canRead())
	  {
	     ...
	     localManage
		 .setFilePath(mFile.getPath(), Const.cmd_Local_List_Go);
	  }
	  else
   ...
   public void onClick(View v)
   {
      ...
      case R.id.btCopy:
         ...
         SysEng.getInstance().addHandlerEvent(new copyFileEvent(m_act,localManage.getFileList()));
         break;
      case R.id.btCut:
         // MobclickAgent.onEvent(m_act, "localEvent","cut");
         SysEng.getInstance().addHandlerEvent(new cutFileEvent(m_act,localManage.getFileList()));
         break;
      case R.id.btDelete:
...
//         }
      ...
      case R.id.btSelectVisible:
         MobclickAgent.onEvent(m_act, "localEvent", "SelVisible");
         Selected(fileAdapter.isSelectVisible());
      }
   }
   
   private void Selected(boolean nFlag)
   {
      fileAdapter.setSelected(!nFlag);
      fileAdapter.notifyDataSetChanged();
      if (!nFlag)
      {
   ...
   private void SwitchStyle(int nStyle)
   {
      ...
      if (nStyle == 1)
      {
         ...
         btLocalSelectAll.setVisibility(View.GONE);
      ...
      else
      {
         ...
         btLocalSelectAll.setVisibility(View.VISIBLE);
         btListStyle
   ...
   public boolean onOptionsItemSelected(MenuItem item)
   {
	  ...
	  SysEng.getInstance().addHandlerEvent(
         ...
         {
	  new SearchFileDialog().Show(m_act, localManage.getCurrentPath(),
	        mSearchPage.getParam(), this);
         }
         break;
      default:
