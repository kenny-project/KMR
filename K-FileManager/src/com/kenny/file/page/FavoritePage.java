      ...
      OnItemLongClickListener
{
   ...
   private TextView tvPic, tvMusic, tvVideo, tvDocument, tvApp, tvZip,
         tvWebPage, tvMessage, tvSDFileStatus;
   ...
   private void SwitchStyle(int bFlag)
   {
      if (bFlag == FLAG_GROUP)
      {
         ...
         btListStyle.setBackgroundResource(R.drawable.file_manager_browser_listmode);
   ...
   private OnScrollListener m_localOnScrollListener = new OnScrollListener()
   {
	  ...
	  if (mFileAdapter != null) mFileAdapter.setShowLogo(false);
	  break;
   ...
   private void FavoriteGroupInit()
   {
	  ...
	  long size = SaveData
	        .Read(m_act, "FavGroupSize_" + temp.getId(), 0l);
	  int count = SaveData.Read(m_act, "FavGroupCount_" + temp.getId(),
	        temp.getCount());
	  temp.setCount(count);
	  temp.setSize(size);
	  switch (temp.getId())
   ...
   public void onClick(View v)
   {
      ...
      case R.id.btLoading_cancel:
         if (mLoadSDFileEvent != null) mLoadSDFileEvent.Cancel();
	     ...
	     .setPositiveButton(m_act.getString(R.string.ok),
		    ...
		    public void onClick(DialogInterface dialog, int which)
		    {
		       mNowGItem.setCount(mNowGItem.getCount()
		             - mDelFiles.size());
		       SaveData.Write(m_act,
		             "FavGroupCount_" + mNowGItem.getId(),
		             mNowGItem.getCount());
		       SysEng.getInstance().addEvent(
