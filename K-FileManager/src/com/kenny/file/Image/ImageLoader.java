...
//   }
   public Drawable loadDrawable(final FileBean fileBean,
         final ImageCallback callback)
   {
      // return loadDrawable(fileBean.getFilePath(), fileBean.getFileEnds(),
      // callback);
 ...
 public class LoadFileIcoEvent
   {
	  ...
	  if (softReference != null)
	  {
	     ...
	     || fileEnds.equals("bmp"))
         {
	  // Long len= fileBean.getFile().length()/1024;
	  // int sampleSize=len.intValue()/5;
	  ...
	  image = new BitmapDrawable(temp);
         ...
         {
	  image = T.getApkFileInfo(m_ctx, new File(path));// fileBean.getFile()
         }
         if (image != null)
         {
	  SoftReference<Drawable>softReferece= new SoftReference<Drawable>(image);
