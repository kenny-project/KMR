...
import com.kenny.file.Adapter.AppAdapter.ViewHolder;
...
import com.kenny.file.util.PkgSizeObserver;
...

{
         ...
         boolean isLastChild, View convertView, ViewGroup viewgroup)
   {
      ...
      if (temp != null)
      {
         ...
         int nSdcard = temp.getFlags() & ApplicationInfo.FLAG_EXTERNAL_STORAGE;// 判断是否存放在SD卡上.
         if (nSdcard != 0)
         {
	  ...
	  viewHolder.mISDV.setVisibility(View.GONE);
         ...
         viewHolder.mCB.setOnClickListener(new OnKCheckedChangeListener(temp));
	  ...
	  if (bShowLogo)
	  {
	     ...
	     Drawable draw = null;
	     draw = mLogoImage.loadDrawable(temp, new KImageCallback(temp,
		 viewHolder));
	     // draw = mLogoImage.loadApp(mContext, temp.getPackageName());
	     if (draw != null)
	     {
...
//   }
         ...
         View convertView, ViewGroup parent)
   {
      ...
      if (convertView == null)
      {
	     ...
	     .inflate(R.layout.listgroup_item, parent, false);
      }
