...
//	protected static InetAddress serverAddress = null;
	...
	private boolean loadSettings() {
		...
		acceptNet = settings.getBoolean(FtpConfigure.ACCEPT_NET,
									    Defaults.acceptNet);
		acceptWifi = settings.getBoolean(FtpConfigure.ACCEPT_WIFI,
										 Defaults.acceptWifi);
		fullWake = settings.getBoolean(FtpConfigure.STAY_AWAKE,
										 Defaults.stayAwake);
		...
		String chrootDir = settings.getString(FtpConfigure.CHROOTDIR,
				Defaults.chrootDir);
	...
	private void setupNotification() {
		...
		if (address != null)
		{
		 ...
		 contentText = getString(R.string.notif_text)+strAddress;
		}
		
		Intent notificationIntent = new Intent(this, FtpServer.class);
	...
	public void run() {
		...
		if(acceptWifi) {
			...
			try {
				...
				myLog.l(Log.WARN, "该端口已被占用, 请更换其它端口.");
//				serverAddress = null;
				cleanupAndStopService();
				return;
			}
